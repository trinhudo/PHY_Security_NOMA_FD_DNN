...
for xx = 1:length(PE_dB)
    ...
    for yy = 1:length(PS_dB)
        for zz = 1:length(thetaN)
            ...
            %
            snrSbN_xN = PS(yy)*thetaN(zz)*gSN./(PS(yy)*beta*thetaF(zz)*gSN + PE(xx)*gEN + 2);
            snrSbN_xF = PS(yy)*thetaF(zz)*gSN./(PS(yy)*thetaN(zz)*gSN + PE(xx)*gEN + 2);
            %
            snrSbF_xF = PS(yy)*thetaF(zz)*gSF./(PS(yy)*thetaN(zz)*gSF + PE(xx)*gEF + 2);
            %
...
save data_systemSOP_PE_MTS.dat min_SystemSOP -ascii
