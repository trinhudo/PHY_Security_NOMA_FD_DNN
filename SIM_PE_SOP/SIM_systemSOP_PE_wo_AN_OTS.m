...
for xx = 1:length(PE_dB)
    ...
    for yy = 1:length(PS_dB)
        for zz = 1:length(thetaN)
            ...
            gEF = abs(hEF).^2;
            %
            snrSiN_xN = PS(yy)*thetaN(zz)*giN./(PS(yy)*beta*thetaF(zz)*giN + PE(xx)*gEN + 1);
            snrSiN_xF = PS(yy)*thetaF(zz)*giN./(PS(yy)*thetaN(zz)*giN + PE(xx)*gEN + 1);
            %
            snrSiF_xF = PS(yy)*thetaF(zz)*giF./(PS(yy)*thetaN(zz)*giF + PE(xx)*gEF + 1);
            %
...
save data_systemSOP_PE_wo_AN_OTS.dat min_SystemSOP -ascii
