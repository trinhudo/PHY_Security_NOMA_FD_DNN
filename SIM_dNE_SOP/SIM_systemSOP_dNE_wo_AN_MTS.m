...
tic; 
for xx = 1:length(dNE)
    fprintf('Running Simulation: dNE = %d (dB) \n', dNE(xx))
    for yy = 1:length(PS_dB)
        for zz = 1:length(thetaN)
            ...
            %
            snrSbN_xN = PS(yy)*thetaN(zz)*gSN./(PS(yy)*beta*thetaF(zz)*gSN + PE*gEN + 1);
            snrSbN_xF = PS(yy)*thetaF(zz)*gSN./(PS(yy)*thetaN(zz)*gSN + PE*gEN + 1);
            %
            snrSbF_xF = PS(yy)*thetaF(zz)*gSF./(PS(yy)*thetaN(zz)*gSF + PE*gEF + 1);
            %
...
save data_systemSOP_dNE_wo_AN_MTS.dat min_SystemSOP -ascii
