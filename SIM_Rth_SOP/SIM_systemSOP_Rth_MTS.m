...
for xx = 1:length(Rth_xN)
    ...
    for yy = 1:length(PS_dB)
        for zz = 1:length(thetaN)
            ...
            for ss = 1:SimTimes
                % Near User
                if(Cs_xN(ss) < Rth_xN(xx) && CSbN_xF(ss) >= Rth_xF(xx))
                    sim_sop_xN = sim_sop_xN + 1;
                end
                if(CSbN_xF(ss) < Rth_xF(xx))
                    sim_sop_xN = sim_sop_xN + 1;
                end
                
                % Far User
                if(Cs_xF(ss) < Rth_xF(xx))
...
%
semilogy(Rth_xN, min_SystemSOP, '-ks')
xlabel({'$\bar{\gamma}_{\mathsf{S}}$'},'Interpreter','latex')
ylabel('System SOP')


save data_systemSOP_Rth_MTS.dat min_SystemSOP -ascii
