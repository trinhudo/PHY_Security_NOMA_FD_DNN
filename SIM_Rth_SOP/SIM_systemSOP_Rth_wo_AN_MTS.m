...
for xx = 1:length(Rth_xN)
    fprintf('Running Simulation: Rth = %d (dB) \n', Rth_xN(xx))
    ...
    % save the system SOP
    min_SystemSOP_MTS(xx) = TSOP(idx_PS, idx_thetaN);
    fprintf('Fing the optimal point (%f, %f)\n', opt_thetaN, opt_PS_dB)
end

semilogy(Rth_xN, min_SystemSOP_MTS, '--ks')
xlabel({'$\bar{\gamma}_{\mathsf{E}}$'},'Interpreter','latex')
ylabel('System SOP')

toc;

save data_systemSOP_Rth_wo_AN_MTS.dat min_SystemSOP_MTS -ascii
