...
%             end
%             %
%             snrSbN_xN = PS(ss)*thetaN(aa)*gSN./(PS(ss)*beta(bb)*thetaF(aa)*gSN + PE*gEN + 2);
...
% 
% save test_PS_thetaN_SystemSOP_MTS.dat TSOP -ascii
%
load test_PS_thetaN_SystemSOP_MTS.dat
PS_dB = -20:1:60;
PS = 10.^(PS_dB./10);
thetaN = 0.01:0.01:0.5;
%
[idx_PS,idx_thetaN]=find(test_PS_thetaN_SystemSOP_MTS==min(test_PS_thetaN_SystemSOP_MTS(:)));
opt_thetaN = thetaN(idx_thetaN);
opt_PS = PS(idx_PS);
opt_PS_dB = PS_dB(idx_PS);
min_SystemSOP = test_PS_thetaN_SystemSOP_MTS(idx_PS, idx_thetaN);
