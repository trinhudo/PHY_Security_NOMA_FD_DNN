% CDE: cell-edge user's SOP with MTS scheme
clear; clc; close all;
...
for ss = 1:length(SNR_dB)
    ...
    a3 = MM*PF*lFE;
    %
    omega4 =  thetaF/thetaN - (gth_xF -1);
    % After Quadrature
    test2 = 0;
    for rr = 1:RR
        xr = cos((2*rr-1)*pi/RR);
        xx = omega4*xr/2 + omega4/2;
        %
        test2 = test2 + omega4*pi/2/RR *sqrt(1 - xr^2) *SNR(ss)*(thetaF - thetaN*(xx + (gth_xF-1)))*lSF * ...
    ...
    Approx_MTS_SOP_xF(ss) = 1 - test2;
    ANA_SOP_xF(ss) = Approx_MTS_SOP_xF(ss);
end
%
...
% save data_SNR_SOP_FarUserMTS_sim.dat SIM_SOP_xF -ascii
save data_SNR_SOP_FarUserMTS_ana.dat ANA_SOP_xF -ascii
