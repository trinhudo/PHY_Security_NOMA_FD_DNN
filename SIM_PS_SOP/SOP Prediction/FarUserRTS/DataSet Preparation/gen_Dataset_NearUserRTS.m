...
for ii = 1:SampleNumber
    ...
    SOP(ii,1) = SimFarUserRTS(MM(ii), PS_dB(ii), PN_dB(ii), PF_dB(ii), PE_dB(ii), dSN(ii), dSF(ii), dSE(ii), dNE(ii), dFE(ii), thetaN(ii), Rth_xN(ii), Rth_xF(ii), SimTimes);
end
SampleData = [MM, PS_dB, PN_dB, PF_dB, PE_dB, dSN, dSE, dNE, dFE, thetaN, Rth_xN, Rth_xF, SOP];

csvwrite('DataSet_FarUserRTS_Kyusung_test1.csv',SampleData);
aa = toc;
ss=seconds(aa);
