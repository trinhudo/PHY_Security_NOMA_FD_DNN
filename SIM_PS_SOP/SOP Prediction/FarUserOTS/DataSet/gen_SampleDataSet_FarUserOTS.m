...
%
data_set = [];
fprintf('Start the sample dataset generating!!! \n')
for ii = 1:SampleNumber
    if(mod(ii, 5e2) == 0)
        fprintf('Running %d over %d \n', ii, SampleNumber)
    end
    SOP(ii,1) = SimFarUserOTS(MM(ii), PS_dB(ii), PN_dB(ii), PF_dB(ii), PE_dB(ii), dSN(ii), dSF(ii), dSE(ii), dNE(ii), dFE(ii), thetaN(ii), beta(ii), Rth_xN(ii), Rth_xF(ii), SimTimes);
    ...
    if mod(ii,step)==0
        ...
        filename = ['DataSet_FarUserOTS_',num2str(ii),'.csv'] ;
...
for kk = step:step:SampleNumber
    filename = ['DataSet_FarUserOTS_',num2str(kk),'.csv'] ;
    all_csv = [all_csv; csvread(filename)]; % Concatenate vertically
end
csvwrite('DataSet_FarUserOTS_1e4.csv', all_csv);
