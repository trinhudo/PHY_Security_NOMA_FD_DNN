...
for ss = 1:length(SNR_dB)
    ...
    snrSiN_xN = SNR(ss)*thetaN*giN./(SNR(ss)*beta*thetaF*giN + PE*gEN + 1);
    snrSiN_xF = SNR(ss)*thetaF*giN./(SNR(ss)*thetaN*giN + PE*gEN + 1);
    %
    snrSiF_xF = SNR(ss)*thetaF*giF./(SNR(ss)*thetaN*giF + PE*gEF + 1);
    %
    snrSiE_xN = SNR(ss)*thetaN*giE./(2);
    snrSiE_xF = SNR(ss)*thetaF*giE./(2);
...
save data_SNR_SOP_NearUserOTS_wo_AN.dat SIM_SOP_xN -ascii
save data_SNR_SOP_FarUserOTS_wo_AN.dat SIM_SOP_xF -ascii
