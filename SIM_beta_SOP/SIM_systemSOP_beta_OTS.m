...
for xx = 1:length(beta)
    fprintf('Simulation is running : beta = %d (dB)\n', beta(xx))
    for yy = 1:length(PS_dB)
        for zz = 1:length(thetaN)
            ...
            gEF = abs(hEF).^2;
            %
            snrSiN_xN = PS(yy)*thetaN(zz)*giN./(PS(yy)*beta(xx)*thetaF(zz).*giN + PE*gEN + 2);
            snrSiN_xF = PS(yy)*thetaF(zz)*giN./(PS(yy)*thetaN(zz)*giN + PE*gEN + 2);
            %
            snrSiF_xF = PS(yy)*thetaF(zz)*giF./(PS(yy)*thetaN(zz)*giF + PE*gEF + 2);
            %
    ...
    min_SOP_xN(xx) = SOP_xN(idx_PS, idx_thetaN);
    min_SOP_xF(xx) = SOP_xF(idx_PS, idx_thetaN);
    min_SystemSOP(xx) = TSOP(idx_PS, idx_thetaN);
end
%
semilogy(beta, min_SOP_xN, '-rs')
hold on
semilogy(beta, min_SOP_xF, '-bs')
hold on
semilogy(beta, min_SystemSOP, '-k^')
xlabel({'$\bar{\gamma}_{\mathsf{S}}$'},'Interpreter','latex')
ylabel('System SOP')

save data_SOP_beta_NearUserOTS.dat min_SOP_xN -ascii
save data_SOP_beta_FarUserOTS.dat min_SOP_xF -ascii
save data_systemSOP_beta_OTS.dat min_SystemSOP -ascii
